      ...
      <div class="modal-body">
        ...
        <div class="mb-3">
            ...
            <div class="btn-group w-100" id="c_payTypeGroup" role="group">
                ...
                <button type="button" class="btn btn-outline-primary c-pay-type-btn" data-type="3" onclick="collectSelectPayType(this)">
                    <i class="fa-solid fa-shuffle fa-xs me-1"></i>Mixed
                </button>
            </div>
        </div>
        ...
        <div class="mb-3">
            ...
            <input type="number" step="0.01" min="0.01" id="c_payNow" class="form-control fg-inp" placeholder="Enter amount">
...
function submitCollect() {
    ...
    if (payNow > balance + 0.001) {
        $('#c_payError').text('Amount cannot exceed balance of \u20b9' + balance.toFixed(2)).show(); return;
    }
    $('#c_payError').hide();
    ...
    Swal.fire({
        ...
        html: 'Collect <strong>\u20b9' + payNow.toFixed(2) + '</strong> for bill <strong>' + $('#c_invoiceNo').val() + '</strong>?',
        ...
        $.post(contextPath + '/logistics/balanceCollection/savePayment.jsp', {
            billId: billId, payNow: payNow, payType: payType, payMode: payMode
        }, function(res) {
            res = $.trim(res);
            if (res === 'OK') {
...
function showHistory(billId, invoiceNo) {
    ...
    $.getJSON(contextPath + '/logistics/balanceCollection/getPaymentHistory.jsp', { billId: billId }, function(data) {
        ...
        html += '<thead class="table-dark"><tr><th>#</th><th>Type</th><th>Payment Mode</th><th>Amount (&#8377;)</th><th>Collected By</th><th>Date &amp; Time</th></tr></thead><tbody>';
        var typeNames = ['','Cash','Bank','Mixed'];
        ...
        if (data.payments && data.payments.length > 0) {
            ...
            html += '<tr><td colspan="5" class="text-center text-muted">No payment records found.</td></tr>';
        }
